...
LINES = {
    ...
    # ★追加：あだ名を決定したときのセリフ ({name} があだ名に置き換わります)
    "nickname_confirm": [
        "わかりました♪ 今日から「{name}」ってお呼びしますね〜。",
    ],

    "rps_start": [
        "いいですよ〜。では、グー / チョキ / パー、どれにするか選んでください。",
        "わかりました〜。楽しく勝負しましょう♪ グー / チョキ / パーの中から選んでくださいね。",
    ],
    "rps_win": [
...
def _pick_high_bucket(level: int) -> str | None:
    if level >= 6: return "high_l6"
    if level == 5: return "high_l5"
    if level == 4: return "high_l4"
    if level == 3: return "high_l3"
    if level == 2: return "high_l2"
    if level == 1: return "high_l1"
    return None
...
def get_reply(message: str, affection_level: int) -> str:
    ...
    return f"{CHAR_NAME}のセリフが設定されていません。"

# ★この関数がないとエラーになります
def get_nickname_line(action: str) -> str:
    """
    action: "ask" (聞くとき) or "confirm" (決まったとき)
    """
    if action == "ask":
        return random.choice(LINES.get("nickname_ask", ["あだ名を教えてください〜。"]))
    if action == "confirm":
        return random.choice(LINES.get("nickname_confirm", ["わかりました。「{name}」ですね♪"]))
    return ""